...
/*    if (argc != 5)
    ...
    std::string res = readFileIntoString(path);
    std::cerr << "file cont:" << res;
    */
    //auto address = net::ip::make_address(argv[1]);
    auto address = net::ip::make_address("127.0.0.1");
    unsigned short port = 8083;//static_cast<unsigned short>(std::atoi(argv[2]));
    auto doc_root = argv[3];
    const unsigned num_threads = std::thread::hardware_concurrency();
    auto const threads = std::max<int>(1, num_threads);//std::atoi(argv[4]));
